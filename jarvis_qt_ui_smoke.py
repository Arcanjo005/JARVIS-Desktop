...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix="jarvis-qt-smoke-") as td:
        os.environ["JARVIS_APP_DIR"]=td; window=JarvisGUI(Logger(),object(),Core()); window.resize(1280,780); window.show(); wait(220)
        ...
        check(window.overlay.isVisible(),"UI overlay is not visible")
        check(window.stack.currentWidget() is window.overlay,"UI overlay is not the active top stack layer")
        ...
        orb_image=window.scene.orb.pixmap().toImage()
        check(orb_image.hasAlphaChannel(),"sphere crop has no alpha channel")
        check(orb_image.pixelColor(0,0).alpha()==0,"sphere still has a square opaque corner")
