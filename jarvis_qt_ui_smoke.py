...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix="jarvis-qt-smoke-") as td:
        ...
        check(not window.composer.plus.icon().isNull(),"approved plus icon missing"); check(not window.composer.mic.icon().isNull(),"approved mic icon missing"); check(not window.composer.send.icon().isNull(),"approved send icon missing"); check(not window.update_button.icon().isNull(),"approved update icon missing")
