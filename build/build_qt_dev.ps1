...
function Require-Path([string]$Path, [string]$Label) {
    if (!(Test-Path -LiteralPath $Path)) { throw "Missing: $Label -> $Path" }
}
...
python tools/restore_ui_assets.py
if ($LASTEXITCODE -ne 0) { throw "restore_ui_assets failed" }

$LegacyVisualFiles = @(
    "gui_qt_dev.py",
    "gui_qt_integrated.py",
    "gui_qt_reference.py",
    "gui_qt_reference_v2.py",
    "gui_reference_exact.py",
    "gui_reference_exact_v2.py",
    "gui_reference_exact_v3.py",
    "gui_reference_final_1311.py",
    "gui_reference_final_1312.py",
    "gui_reference_release_1312.py",
    "jarvis_reference_orb.py",
    "jarvis_reference_scene_139.py",
    "jarvis_visual_runtime.py"
)
$FoundLegacy = @($LegacyVisualFiles | Where-Object { Test-Path -LiteralPath (Join-Path $Root $_) })
if ($FoundLegacy.Count -gt 0) {
    throw "Legacy visual sources still exist in build workspace: $($FoundLegacy -join ', ')"
}
...
foreach ($item in @(
    @($MainPath, "main.py"), @($GuiPath, "gui.py"), @($IconFile, "jarvis.ico"),
    @($VersionFile, "version_info"), @($DataDir, "data"), @($PluginsDir, "plugins"),
    @($AssetsDir, "approved assets"), @($UpdateConfig, "update_config"), @($IssFile, "Inno script")
)) { Require-Path $item[0] $item[1] }

$RequiredAssets = @(
    "workspace_bg.jpg", "sphere_3d.webp"
)
foreach ($Asset in $RequiredAssets) {
    Require-Path (Join-Path $AssetsDir $Asset) "approved UI asset $Asset"
}

$MainSource = Get-Content -LiteralPath $MainPath -Raw
if (-not $MainSource.Contains("from gui import JarvisGUI")) {
    throw "main.py is not pointing directly to the single-source gui.py"
}
$GuiSource = Get-Content -LiteralPath $GuiPath -Raw
foreach ($Token in @("from gui_qt_", "from gui_reference", "JARVIS DESKTOP INTELLIGENCE", "_paint_hologram", "QRadialGradient")) {
    if ($GuiSource.Contains($Token)) { throw "Forbidden legacy UI token in gui.py: $Token" }
}
...
$Args = @(
    ...
    "--collect-all", "sounddevice", "--collect-all", "vosk", "--collect-submodules", "edge_tts",
...
foreach ($Asset in $RequiredAssets) {
    $Args += @("--add-data", "$((Join-Path $AssetsDir $Asset));assets")
}
...
foreach ($Asset in $RequiredAssets) {
    Require-Path (Join-Path $BundledAssets $Asset) "bundled approved UI asset $Asset"
}
...
if ($Runtime.ExitCode -ne 0) {
    ...
    throw "Runtime selftest Qt Dev failed: $($Runtime.ExitCode)"
}
$Report = Get-Content -LiteralPath $RuntimeReport -Raw | ConvertFrom-Json
if (-not $Report.ok -or -not $Report.checks.pyside6 -or -not $Report.checks.single_qt_ui) {
    Write-Host (Get-Content -LiteralPath $RuntimeReport -Raw)
...
Require-Path $Installer "Qt Dev installer"
