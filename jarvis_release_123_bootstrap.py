"""JARVIS Desktop 1.2.3 Chat Core / Blue Core non-blocking UI layer.
...
RELEASE_LAYER = "1.2.3-nonblocking-ui1"
...
def _pyinstaller_collect_dynamic_modules():
    ...
    import jarvis_ui_guard  # noqa: F401
...
def _patch_gui_nonblocking() -> None:
    """1.2.3: protect Tk from heavy runtime/background work.

    This patch does NOT change JarvisCore/Gemini or command routing.
    """
    mod = sys.modules.get("gui")
    if mod is None or "gui-nonblocking-123" in _PATCHED:
        return
    cls = getattr(mod, "JarvisGUI", None)
    if cls is None:
        return
    ...
    # ------------------------------------------------------------------
    # 1) Stagger heavy boot jobs. Voice and Qt overlay are lazy.
    # ------------------------------------------------------------------
    def _v123_start_worker(self, name, target):
        if not callable(target):
            return None

        def runner():
            guard = getattr(self, "_v123_ui_freeze_guard", None)
            try:
                if guard:
                    guard.mark(f"worker-start:{name}")
                target()
            except Exception as exc:
                _safe_log(self, "warning", f"Worker {name} falhou: {exc}", "BOOT")
            finally:
                try:
                    if guard:
                        guard.mark(f"worker-end:{name}")
                except Exception:
                    pass
        ...
        try:
            ...
            _safe_log(self, "warning", f"Nao consegui iniciar {name}: {exc}", "BOOT")
            return None

    def _v123_start_deferred_runtime(self):
        # The 1.1.2 build launched eight expensive threads at the same instant.
        # On some Windows/audio/driver combinations this can starve Tk long
        # enough for Windows to mark the entire process "Not responding".
        jobs = [
            (350,  "JARVIS-BOOT-AI",       getattr(self.core, "prewarm", lambda: None)),
            (1200, "JARVIS-BOOT-LEARNING", getattr(self, "_setup_learning_runtime", lambda: None)),
            (2400, "JARVIS-BOOT-ADVANCED", getattr(self, "_setup_advanced_modules", lambda: None)),
            (3800, "JARVIS-BOOT-DESKTOP",  getattr(self, "_setup_desktop_integration", lambda: None)),
            (6500, "JARVIS-BOOT-APPINDEX", getattr(self.actions, "rebuild_app_index", lambda: None)),
            (9000, "JARVIS-BOOT-UPDATER",  getattr(self, "_background_update_check", lambda: None)),
        ]
        for delay_ms, name, target in jobs:
            try:
                self.root.after(
                    int(delay_ms),
                    lambda n=name, t=target: _v123_start_worker(self, n, t),
                )
            except Exception as exc:
                _safe_log(self, "warning", f"Agendamento {name} falhou: {exc}", "BOOT")

        # Voice/Whisper and Qt overlay are intentionally NOT auto-started.
        # They start on first voice interaction, keeping text chat independent
        # from PortAudio/Whisper/Qt driver problems.
        _safe_log(
            self,
            "info",
            "Boot 1.2.3: voz/Whisper/overlay em modo sob demanda; chat tem prioridade.",
            "BOOT",
        )

    cls._start_deferred_runtime = _v123_start_deferred_runtime
    cls._v123_start_worker = _v123_start_worker

    # ------------------------------------------------------------------
    # 2) Lazy voice. Clicking voice starts it in background, never waits.
    # ------------------------------------------------------------------
    def _v123_ensure_voice_runtime(self):
        if getattr(self, "voice_engine", None) is not None:
            return True
        if bool(getattr(self, "_v123_voice_starting", False)):
            return False
        self._v123_voice_starting = True

        def setup():
            try:
                getattr(self, "_setup_voice", lambda: None)()
                # Start the Qt helper only after voice setup had a chance to
                # initialize; it remains isolated from Tk.
                try:
                    getattr(self, "_setup_qt_voice_overlay", lambda: None)()
                except Exception:
                    pass
            finally:
                self._v123_voice_starting = False
                try:
                    self._post_ui_event("runtime_ready", "voice-lazy")
                except Exception:
                    pass

        _v123_start_worker(self, "JARVIS-LAZY-VOICE", setup)
        return False

    cls._v123_ensure_voice_runtime = _v123_ensure_voice_runtime

    original_toggle_voice = getattr(cls, "_toggle_voice_visual_mode", None)
    if callable(original_toggle_voice):
        def _v123_toggle_voice_visual_mode(self):
            _v123_ensure_voice_runtime(self)
            return original_toggle_voice(self)
        cls._toggle_voice_visual_mode = _v123_toggle_voice_visual_mode

    original_voice_click = getattr(cls, "_voice_visual_click", None)
    if callable(original_voice_click):
        def _v123_voice_visual_click(self):
            if getattr(self, "voice_engine", None) is None:
                _v123_ensure_voice_runtime(self)
                try:
                    self._set_voice_overlay_text("Preparando voz em segundo plano...")
                except Exception:
                    pass
                return None
            return original_voice_click(self)
        cls._voice_visual_click = _v123_voice_visual_click

    # ------------------------------------------------------------------
    # 3) Reminder database polling off the Tk thread.
    # ------------------------------------------------------------------
    def _v123_start_persistent_reminder_watcher(self):
        if bool(getattr(self, "_v123_reminder_started", False)):
            return
        self._v123_reminder_started = True

        def schedule_next(delay_ms=10000):
            try:
                self.root.after(int(delay_ms), launch)
            except Exception:
                pass
        ...
        def launch():
            if bool(getattr(self, "_exit_requested", False)):
                return
            if bool(getattr(self, "_v123_reminder_running", False)):
                schedule_next(4000)
                return
            self._v123_reminder_running = True

            def worker():
                items = []
                try:
                    items = list(self.memory_store.due_reminders() or [])
                    for item in items:
                        try:
                    ...
                    _safe_log(self, "warning", f"Lembretes em background: {exc}", "MEMORY")
                finally:
                    self._v123_reminder_running = False
                ...
                for item in items[:20]:
                    try:
                        self._post_ui_call(
                            ...
                            f"LEMBRETE: {item.get('task', '')}",
                            is_jarvis=True,
                            speak=bool(getattr(self, "voice_enabled", False)),
                        )
                    except TypeError:
                        # _post_ui_call only carries positional args on older builds.
                        try:
                            self._post_ui_event(
                                "ui_call",
                                lambda it=item: self.add_message(
                                    getattr(mod, "PUBLIC_NAME", "JARVIS"),
                                    f"LEMBRETE: {it.get('task', '')}",
                                    is_jarvis=True,
                                    speak=bool(getattr(self, "voice_enabled", False)),
                                ),
                            )
                            ...
                            pass
                try:
                    self._post_ui_event("ui_call", lambda: schedule_next(10000))
                except Exception:
                    pass

            _v123_start_worker(self, "JARVIS-REMINDER-CHECK", worker)

        try:
            self.root.after(1800, launch)
        except Exception:
            pass

    cls._start_persistent_reminder_watcher = _v123_start_persistent_reminder_watcher

    # ------------------------------------------------------------------
    # 4) Proactive player watcher is disabled by default for 1.2.3 stability.
    #    All explicit media commands remain available.
    # ------------------------------------------------------------------
    original_player_watcher = getattr(cls, "_start_jarvis_player_watcher", None)
    def _v123_start_jarvis_player_watcher(self):
        enabled = str(os.environ.get("JARVIS_PROACTIVE_PLAYER_WATCHER", "0")).strip().lower()
        if enabled in {"1", "true", "on", "sim"} and callable(original_player_watcher):
            # Even opt-in starts after the rest of the UI has settled.
            try:
                self.root.after(
                    15000,
                    lambda: _v123_start_worker(
                        self,
                        "JARVIS-PROACTIVE-PLAYER",
                        original_player_watcher.__get__(self, cls),
                    ),
                )
            except Exception:
                pass
            ...
            _safe_log(
                self,
                "info",
                "Player proativo suspenso no boot 1.2.3; comandos de midia continuam ativos.",
                "BOOT",
            )
    cls._start_jarvis_player_watcher = _v123_start_jarvis_player_watcher

    # ------------------------------------------------------------------
    # 5) Bound each UI queue drain to ~9 ms / 16 events.
    # ------------------------------------------------------------------
    original_drain = getattr(cls, "_drain_ui_events", None)
    if callable(original_drain):
        def _v123_drain_ui_events(self):
            q = getattr(self, "_ui_event_queue", None)
            begin = getattr(q, "begin_budget", None)
            end = getattr(q, "end_budget", None)
            guard = getattr(self, "_v123_ui_freeze_guard", None)
            try:
                if guard:
                    guard.mark("drain-ui-events")
                if callable(begin):
                    begin(max_items=16, max_seconds=0.009)
                ...
                try:
                    if guard:
                        guard.mark("mainloop-idle")
                except Exception:
                    pass
        cls._drain_ui_events = _v123_drain_ui_events

    # ------------------------------------------------------------------
    # 6) Install heartbeat after the original GUI is fully built.
    # ------------------------------------------------------------------
    original_init = getattr(cls, "__init__", None)
    if callable(original_init):
        def _v123_init(self, *args, **kwargs):
            original_init(self, *args, **kwargs)
            try:
                from jarvis_ui_guard import install_gui_responsiveness_guard
                install_gui_responsiveness_guard(self)
                _safe_log(
                    self,
                    "info",
                    "UI Freeze Guard 1.2.3 ativo; fila Tk limitada e heartbeat iniciado.",
                    "GUI",
                )
            except Exception as exc:
                _safe_log(self, "warning", f"UI Freeze Guard indisponivel: {exc}", "GUI")
        cls.__init__ = _v123_init

    _PATCHED.add("gui-nonblocking-123")
